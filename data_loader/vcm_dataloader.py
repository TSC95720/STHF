...
class VideoDataset_train(Dataset):
    ...
    def __init__(self, dataset_ir,dataset_rgb, seq_len=12, sample='evenly', transform0=None, transform1=None,transform2=None,transform3=None, index1=[], index2=[]):
    ...
    def __getitem__(self, index):
        ...
        if self.sample == 'random':
            ...
            for index in number_ir:
                index = int(index)
                img_path = img_ir_paths[index]
                img_path_i = '/dataset/Inversing_dataset/TensoIR-VCM_Fin/' + img_path[39:]
                # img_path_i = '/home/z/文档/test/TSC/dataset/HITSZ-VCM_Rec/' + img_path[38:]

                img = read_image(img_path)
                img_i = read_image(img_path_i)

                if self.transform3 is not None:
                    img = self.transform3(img)
                    img_i = self.transform0(img_i)

                imgs_ir.append(img)
                imgs_ir_i.append(img_i)

            imgs_ir = torch.cat(imgs_ir, dim=0)
            imgs_ir_i = torch.cat(imgs_ir_i, dim=0)
                ...
                if self.transform1 is not None and self.transform2 is not None:
                    if self.transform1 is not None and self.transform2 is not None:
                        if idx == 0:
                            ...
                            img_i_trans = self.transform0(img_i)
                        elif idx == 1:
                            img_trans = self.transform2(img)
                            img_i_trans = self.transform0(img_i)

                imgs_rgb.append(img_trans)
                imgs_rgb_i.append(img_i_trans)

            imgs_rgb = torch.cat(imgs_rgb, dim=0)
            imgs_rgb_i = torch.cat(imgs_rgb_i, dim=0)

            return imgs_rgb, imgs_rgb_i, imgs_ir, imgs_ir_i, pid_rgb, pid_ir
...
class VideoDataset_test(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.sample == 'video_test':
            ...
            return imgs_ir, imgs_ir_i, pid, camid
        else:
            raise KeyError("Unknown sample method: {}. Expected one of {}".format(self.sample, self.sample_methods))
